...
const path = require('path'); //调用nodejs的path模块
    fs.readFile(path.join(__dirname+,'/111.txt'),"utf-8",function(err,data){    //读取文件
        if (err) {          //err如果为!null，即有保错信息
           ...
           //将替换好的数组每条数据添加换行
            fs.writeFile(path.join(__dirname+,'/111.txt'),newstr,function(err){
                //调用写入，写入数据是处理好的数组。
                if(err){
